...
`;
  ...
  --linearPrimarySecondary: linear-gradient(rgb(${convert.hex.rgb(
    primaryColor
  )}), rgb(${convert.hex.rgb(secondaryColor)}));
  --linearPrimaryAccent: linear-gradient(rgb(${convert.hex.rgb(
    primaryColor
  )}), rgb(${convert.hex.rgb(accentColor)}));
  --linearSecondaryAccent: linear-gradient(rgb(${secondaryColor}), rgb(${convert.hex.rgb(
    accentColor
  )}));
  --radialPrimarySecondary: radial-gradient(rgb(${convert.hex.rgb(
    primaryColor
  )}), rgb(${convert.hex.rgb(secondaryColor)}));
  --radialPrimaryAccent: radial-gradient(rgb(${convert.hex.rgb(
    primaryColor
  )}), rgb(${convert.hex.rgb(accentColor)}));
  --radialSecondaryAccent: radial-gradient(rgb(${secondaryColor}), rgb(${convert.hex.rgb(
    accentColor
  )}));
`;
  const themeColorsGradientsHsl = `
  --linearPrimarySecondary: linear-gradient(hsl(${convert.hex.hsl(
    primaryColor
  )}), hsl(${convert.hex.hsl(secondaryColor)}));
  --linearPrimaryAccent: linear-gradient(hsl(${convert.hex.hsl(
    primaryColor
  )}), hsl(${convert.hex.hsl(accentColor)}));
  --linearSecondaryAccent: linear-gradient(hsl(${secondaryColor}), hsl(${convert.hex.hsl(
    accentColor
  )}));
  --radialPrimarySecondary: radial-gradient(hsl(${convert.hex.hsl(
    primaryColor
  )}), hsl(${convert.hex.hsl(secondaryColor)}));
  --radialPrimaryAccent: radial-gradient(hsl(${convert.hex.hsl(
    primaryColor
  )}), hsl(${convert.hex.hsl(accentColor)}));
  --radialSecondaryAccent: radial-gradient(hsl(${convert.hex.hsl(
    secondaryColor
  )}), hsl(${convert.hex.hsl(accentColor)}));
`;
  return (
    <StyledColorConvertor>
      ...
      <StyledCodeBlock>
        {popupContent === "TailwindContent" ? (
          <StyledCodeFileName>tailwind.config.js</StyledCodeFileName>
        ) : null}
        <StyledCode
          $popupContent={
        ...
        >
          ...
          <div ref={copyRef}>
            ...
            {popupContent === "TailwindContent"
              ? popupSub === null || popupSub === "HEX"
                ? themeColorsHexTailwind
                : popupSub === "RGB"
                ? themeColorsRgbTailwind
                : popupSub === "HSL"
                ? themeColorsHslTailwind
                : null
              : null}
            {/* Css Content */}
            {popupContent === "ScssContent"
            ...
            {popupContent === "GradientsContent"
              ? popupSub === null || popupSub === "HEX"
                ? themeColorsGradients
                : popupSub === "RGB"
                ? themeColorsGradientsRgb
                : popupSub === "HSL"
                ? themeColorsGradientsHsl
                : null
              : null}
          </div>
        </StyledCode>
